...
def nasa_inject_anomalies(
    ...
    if injection_method == "None":
        ...
        # Inject synthetic anomalies: smap injection
...
def nasa_pipeline(
    ...
    **kwargs,
    ...
    #####     Load Data     #####
    train_set, test_set = ncad.datasets.nasa(
        path=data_dir,
        benchmark=benchmark,
    )
    # Transform to univariate problem by keeping only the first dimension of each TimeSeries
    univariate = False
    if univariate:
        N = len(train_set)
        for i in range(N):
    ...
    # Padding time series to the left if it is shorter than window_length
    for i, ts in enumerate(train_set):
        if ts.shape[0] < window_length:
            pad_len = (window_length - ts.shape[0]) + num_crops_per_series
            assert pad_len > 0
            warnings.warn(
                f"Paddign series {i} with {pad_len} timesteps on the left ({np.round(100*pad_len/ts.shape[0],0)} % of its original length)"
            )
            ts_pad = TimeSeries(
                values=np.zeros((pad_len, ts.shape[1])).squeeze(), labels=np.zeros(pad_len)
            )
    ...
    print(f"ncad on nasa {benchmark} dataset finished successfully!")
