...
def nasa(
    ...
    **kwargs,
    ...
    # 补充实验SMAP，只保留第一维度的数据，然后加入一个slow_slop的数据增强模块
    for i in range(len(train_dataset)):
        train_dataset[i].values = train_dataset[i].values[:, 0].reshape((-1, 1))
    for i in range(len(test_dataset)):
        test_dataset[i].values = test_dataset[i].values[:, 0].reshape((-1, 1))
